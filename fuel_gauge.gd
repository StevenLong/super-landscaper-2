...
func _draw() -> void:
	...
	var fs := 18
	draw_string(font, Vector2(0, label_h - 4), "F", HORIZONTAL_ALIGNMENT_CENTER, size.x, fs)
	draw_string(font, Vector2(0, size.y - 4), "E", HORIZONTAL_ALIGNMENT_CENTER, size.x, fs)
