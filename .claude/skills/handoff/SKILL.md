...
HANDOFF.md carries what git cannot: decisions, verdicts, owed checks, and what comes next.
...
- End with OWED CHECKS (if any) and NEXT (a proposed order).
- Plain hyphens only, no em or en dashes.

## 4. Maintain the ledger
The "## Owed checks (rolling ledger)" block holds things only the dev can verify by playing
(feel, readability, whether the face is funny, whether a job is tense).
- New check: a short bold title tagged (SN), then `- [ ]` bullets with one observable each
  ("do X, watch Y").
- Remove bullets the dev confirmed this session; the session entry records the verdict.
- General praise ("all good") does not clear a multi-bullet check. Read the bullets back and
  ask which were exercised, then clear only those.
- If a check has rolled for 3+ sessions, tell the dev rather than rolling it again.
- Empty ledger: a single "Nothing owed." line.

## 5. Trim
Keep about 8 sessions. Delete older entries; git history keeps them.

## 6. Commit and push
HANDOFF.md plus anything uncommitted. No Co-Authored-By trailer. Write the message with a
...
## 7. Report
Two lines on what shipped and what's NEXT, then the full owed checklist verbatim.
