...
1. [BUG, small, BLOCKS] Fuel-can glug plays forever after pouring (S1-ONFOOT).
   ...
   FIX: a separate non-looping one-shot sound for the pour and the plop.
2. [BUG, small, BLOCKS] The truck refills push-mower stamina (S1-PUSH). `truck.gd:21-23`
...
3. [VISUAL, small] A little line like an accent over the W on the title (S1-ATTRACT, S1-FONT).
   Root cause (atlas pixels checked; the on-screen bleed is assumed): in `art/font.png` the
   `g` descender sits on the last row of its cell, directly above the `w` cell, and cells
   touch, so sampling at the title's scale pulls it in. FIX: a blank gutter between atlas
   cells (`tools/make_font.py` CH 10 -> 11 plus `pixel_font.gd`), regenerate.
4. [VISUAL, small] The attract mower visibly flips at each row end (S1-ATTRACT).
   `attract.gd:43` turns 60px past the screen edge, less than the sprite's half-length.
   FIX: turn further off screen.
5. [BUG?, small] F9 (board cash cheat, `board.gd:147`) also seems to trigger something in
   Godot, pausing the run (S1-BOARD). FIX: move the cheat to an unclaimed key.
   QUESTION: what did it do exactly, and were you running from the editor?
6. [FEATURE, small] WASD should drive menus too (S1-MENU-AUDIO). `project.godot` has no
   `ui_up/down/left/right` overrides. FIX: add W/S/A/D to those actions.
7. [FEATURE, small] Dev cheat for the high end (S1-RUN): boost rep, and instantly complete a
   job with max pay and rep. Lets you see medium/large lawns and ponds without grinding.
   FIX: debug-only keys on the board and in a job, next to the F9 cheat.
...
10. [VISUAL, small] The corner face can cover the player (S1-FACE). FIX options: (a) move
   ...
   corner, or fade it, whenever the player gets near it. QUESTION: which.
   Recommend (b): it guarantees the player is never hidden, (a) doesn't.
...
24. [DESIGN] Tone (S1-KO): a lawnmower wouldn't just knock someone out; the PG result
   ...
   CALLS item "the dog is never killed". QUESTION: how far, and does it apply to the dog?
...
26. [DESIGN, large] WANTED doesn't land and ARRESTED never fired (S1-WANTED, S1-FIRED).
   ...
   out after a firing. You asked for ideas; see the chat reply.
27. [DESIGN, large] Shop and board as separate places (S1-BOARD): a walkable shop/showroom
   ...
   outside-the-run track the design doc defers. QUESTION: un-deferring it?
...
PROPOSED ORDER: 1, 2, 3, 4, 5, 6, 7 (one quick-fix batch), then 9, 10, 12, 11, 13, then a
